...
public class Ekit extends JFrame implements WindowListener
{
	...
	private static final long	serialVersionUID	= 7451335804642125840L;
...
//		}
		...
		if(includeToolBar)
		{
			if(multiBar)
			{
				...
				gbc.gridy      = 1;
				contentPane.add(ekitCore.getToolBarMain(includeToolBar), gbc);

				gbc.gridy      = 2;
				contentPane.add(ekitCore.getToolBarFormat(includeToolBar), gbc);

				gbc.gridy      = 3;
				contentPane.add(ekitCore.getToolBarStyles(includeToolBar), gbc);
			...
			else
			{
				...
				contentPane.add(ekitCore.getToolBar(includeToolBar), BorderLayout.NORTH);
			}
		}
		else
		{
	...
	public void windowClosing(WindowEvent we)
	{
		this.dispose();
		synchronized (this) {
			notifyAll(); // notify the waiting main method
		}
	}
