...
public class UnicodeDialog extends JDialog implements ActionListener
{
	...
	private static final long	serialVersionUID	= 4134769887558101373L;
...
*/
	...
	/**
	 * Creates and displays a UnicodeDialog.
	 * @param parent the parent Frame or Dialog of the UnicodeDialog
	 * @param title the dialog title
	 * @param bModal whether the dialog should be modal
	 * @param index the start index
	 * @return the UnicodeDialog instance
	 */
	public static UnicodeDialog open(EkitCore parent, String title, boolean bModal, int index)
	{
		UnicodeDialog me;
		final Window parentWindow = parent.getParentWindow();
		if (parentWindow instanceof Frame)
		{
			me = new UnicodeDialog((Frame) parentWindow, parent, title, bModal, index);
		}
		else if (parentWindow instanceof Dialog)
		{
			me = new UnicodeDialog((Dialog) parentWindow, parent, title, bModal, index);
		}
		else
		{
		...
		me.pack();
		me.populateButtons(index, 0);
		me.setVisible(true);
		return me;
	}

	private UnicodeDialog(Dialog parentDialog, EkitCore parent, String title, boolean bModal, int index)
	{
	...
	private UnicodeDialog(Frame parentFrame, EkitCore parent, String title, boolean bModal, int index)
	{
	...
	/**
	 * @deprecated use {@link #open(EkitCore, String, boolean, int)} instead
	 */
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		else
		{
			...
			catch(BadLocationException ble) { System.out.println("BadLocationException during character insertion : " + ble.getMessage()); }
		}
	}
	...
	public void init(int startIndex)
	{
		...
		for(int counter = 0; counter < UNICODEBLOCKSIZE; counter++)
		{
			...
			buttonArray[counter].setBorder(BorderFactory.createEtchedBorder(EtchedBorder.LOWERED));
			buttonArray[counter].addActionListener(this);
			if(counter == 0)
			{
				FontRenderContext frcLocal = ((Graphics2D)(parentEkit.getGraphics())).getFontRenderContext();
