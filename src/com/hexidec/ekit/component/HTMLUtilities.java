...
*/
	...
	/**
	 * This method checks for the first tag from the current position upwards in the hierarchy. It is looking for one of
	 * the given tags. The first found tag is return. If none is found while the html tag is reached, null is returned.
	 */
	public HTML.Tag checkParentsTag(HTML.Tag[] tag)
	{
		ArrayList listOfTagNames = new ArrayList(tag.length);
		for (int tagIndex = 0; tagIndex < tag.length; tagIndex++) {
			listOfTagNames.add(tag[tagIndex].toString().toLowerCase());
		}
		for (Element e = parent.getExtendedHtmlDoc().getParagraphElement(parent.getCaretPosition());
			!e.getName().equalsIgnoreCase("html");
			e = e.getParentElement())
		{
			final int indexOf = listOfTagNames.indexOf(e.getName().toLowerCase());
			if (indexOf >= 0) {
				return tag[indexOf];
			}
		}
